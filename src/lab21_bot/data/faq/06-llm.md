...
## Выключить LLM (`LLM_ENABLED=false`)

На слабом хосте (например, NAS без видеокарты и с 8 ГБ RAM) модель не поднимают. Тогда в `.env` ставят `LLM_ENABLED=false` и профиль `ollama` не включают. Пайплайн остаётся тот же, но кнопка «Одобрить → LLM» превращается в **«Одобрить исходник»**: текст автора становится черновиком без переписывания, дальше обычная модерация и публикация. Настройки и шаблоны LLM сохраняются и заработают после включения.
